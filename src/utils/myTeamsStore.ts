import type { ApplicantStatus, CollabApplicant, CollabRequest } from "../types/collab";

/**
 * Project team yang DIBUAT oleh user (localStorage), beserta daftar pendaftar.
 * Dipakai di halaman "My teams". Nanti dipindah ke backend.
 */

export interface MyTeam {
  request: CollabRequest;
  applicants: CollabApplicant[];
  /** id server komunitas tim (untuk invite link). */
...
function writeMyTeams(teams: MyTeam[]): void {
  try {
    ...
    // storage penuh (mis. gambar besar) — abaikan
  }
}
...
/** Beberapa pendaftar contoh untuk tim yang baru dibuat. */
