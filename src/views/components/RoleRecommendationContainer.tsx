...
function RoleRecommendationContainer({
  ...
  // Loading -> momen "AI meracik roadmap".
  if (isLoading) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-canvas px-6">
        <div className="flex max-w-md flex-col items-center gap-4 text-center">
          ...
          <h2 className="font-display text-xl font-semibold text-ink">
            Couldn't build your roadmap
          </h2>
          <p className="text-sm text-muted">
            Something went wrong processing your choices. Please try again.
          </p>
          <button
            type="button"
            onClick={onRetry}
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-10">
        ...
        <header data-animate className="flex flex-col gap-3">
          <span className="font-mono text-xs uppercase tracking-[0.2em] text-accent">
            Step 3 of 3 · Your IT role
          </span>
          <h1 className="font-display text-3xl font-bold leading-tight text-ink sm:text-4xl">
            {recommendedRole ? (
              <>
                You'd make a great{" "}
                <span style={{ color: recommendedRole.accent }}>{recommendedRole.title}</span>
              </>
            ) : (
              "Pick your IT role"
            )}
          ...
          <p className="max-w-xl text-sm text-muted sm:text-base">
            This is our recommendation from your questionnaire. Not sure? You're free to pick a
            different role below — the roadmap will adjust.
          </p>
        </header>
          ...
          {roles.map((role) => (
            <div key={role.id} className="border-r border-b border-line">
              <RoleCard
                ...
                recommended={role.id === recommendedId}
      ...
      <footer className="fixed inset-x-0 bottom-0 border-t border-line bg-surface/90 px-6 py-4 backdrop-blur sm:px-8">
        <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-4">
          <p className="hidden text-sm text-muted sm:block">
            {selectedRoleId ? "Role selected — ready for the roadmap!" : "Pick a role to continue."}
          </p>
          <button
            type="button"
            onClick={onConfirm}
            ...
            className={`ml-auto py-2 text-sm font-semibold ${
              selectedRoleId ? "cursor-pointer text-primary" : "cursor-not-allowed text-muted"
            }`}
          >
            <span className="inline-flex items-center gap-1.5">
              Continue to Roadmap <ArrowRightIcon className="h-4 w-4" />
            </span>
          </button>
        </div>
      </footer>
