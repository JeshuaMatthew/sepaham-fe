      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
        ...
        <header className="flex flex-col gap-2">
          ...
          <p className="max-w-xl text-sm text-muted">
            AI-matched careers, companies you can reach out to, and your overall readiness.
          </p>
        </header>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {hasCv ? (
                <span className="inline-flex items-center gap-1.5 text-sm text-muted">
                  ...
                  {cvName ? <span className="text-ink">· {cvName}</span> : null}
                </span>
              ) : (
                <label className="inline-flex cursor-pointer items-center gap-1.5 border border-line px-3 py-1.5 text-sm font-semibold text-primary">
              ...
              {githubConnected ? (
                ...
                <button
                  ...
                  className="inline-flex cursor-pointer items-center gap-1.5 border border-line px-3 py-1.5 text-sm font-semibold text-primary"
                >
        ...
        {/* 1. Top 3 careers — bagian PENTING, dibungkus gradient biru */}
        <section className="grad-blue flex flex-col gap-4 p-6 sm:p-7">
          <div className="flex flex-col gap-1">
            ...
            <h2 className="font-display text-xl font-bold text-ink">Top 3 careers for you</h2>
            ...
            {topMatches.map((match, index) => (
              <div key={match.role.id} className="flex flex-col gap-2 p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="font-display text-lg font-bold text-ink">{match.fitPercent}%</span>
                </div>
                <h3 className="font-display text-base font-semibold text-ink">{match.role.title}</h3>
        ...
        {/* 2. Kontak perusahaan berdasarkan top-3 karier */}
        ...
        {/* 3. Consult AI — pindah ke subhalaman */}
        <button
          type="button"
          onClick={onGoConsult}
          className="grad-blue flex items-center gap-3 border border-blue p-5 text-left"
        >
          <SparkleIcon className="h-6 w-6 shrink-0 text-ink" />
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 border-l border-t border-line sm:grid-cols-2">
            {profile.sources.map((source) => {
              ...
              return (
                <div key={source.key} className="flex flex-col gap-2 border-r border-b border-line p-5">
                  <div className="flex items-center justify-between gap-2">
