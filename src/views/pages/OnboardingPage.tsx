...
import { clearCv, getCv, saveCv } from "../../utils/cv";
...
/**
 * OnboardingPage — dua tahap: (1) minta CV opsional di layar terpisah, lalu
 * (2) kuesioner Likert yang dipecah 5 pertanyaan per halaman. Page mengurus
 * state & navigasi tahap; container hanya render. TIDAK ADA class Tailwind.
 */
...
function OnboardingPage() {
  ...
  const answeredTotal = questions.filter((question) => answers[question.id]).length;
  const currentPageAnswered =
    pageQuestions.length > 0 && pageQuestions.every((question) => answers[question.id]);
  ...
  return (
    <OnboardingContainer
      ...
      onCvContinue={() => setPhase("github")}
      onGithubContinue={() => setPhase("questions")}
      onGithubBack={() => setPhase("cv")}
      onNext={handleNext}
      onBack={handleBack}
      onRetry={() => {
        void refetch();
